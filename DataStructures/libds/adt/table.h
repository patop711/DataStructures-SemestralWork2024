...
namespace ds::adt {
    ...
    //----------
    //----------ModifiedHashTable na osetrenie duplicit----------
    template<typename K, typename T>
    class ModifiedHashTable : public HashTable<K, ds::amt::ImplicitSequence<T>*>
    {
    public:
    ...
    //----------ModifiedHashTable na osetrenie duplicit----------
    template<typename K, typename T>
    void ModifiedHashTable<K, T>::insert(const K& key, const T& data)
    {
        if (this->contains(key))
        {
            this->find(key)->insertLast().data_ = data;
        }
        else
        {
            auto* sekvencia = new ds::amt::ImplicitSequence<T>();
            this->HashTable<K, ds::amt::ImplicitSequence<T>*>::insert(key, sekvencia);
            this->find(key)->insertLast().data_ = data;
        }
    }

    template <typename K, typename T>
    ModifiedHashTable<K, T>::~ModifiedHashTable()
    {
        for (auto& item : *this)
        {
            auto sekvencia = item.data_;
            delete item.data_;
        }
    }
    //----------ModifiedHashTable na osetrenie duplicit----------
